...
def get_labels(lang, sentiment_dir, canonization_labels_dir, label_type):
    ...
    if label_type == 'library':
        ...
        labels = pd.read_csv(sentiment_dir + library_file, sep=";", header=0)[["id", "sum_libraries"]]
        labels = labels.merge(right=canonization_scores, how="left", on="id", validate="one_to_one")
        labels = labels.rename(columns={"sum_libraries": "classified", "file_name": "book_name"})
        ...
        score_labels = pd.read_csv(sentiment_dir + scores_file, sep=";", header=0)
        ...
        class_labels = pd.read_csv(sentiment_dir + class_file, sep=";", header=0)[['textfile', 'journal', 'text_id', 'classified']]

        labels = score_labels.merge(right=class_labels, 
                                    on=['textfile', 'journal', 'text_id'], 
                                    how='outer', 
                                    suffixes=('', '_scorelabels'), 
                                    validate='one_to_one')
        labels = labels[['sentiscore_average', 'sentiment_Textblob', 'text_id', 'classified']]
        ...
        if (label_type == 'textblob') or (label_type == 'sentiart'):
            ...
            labels = labels.groupby("book_name").apply(_average_scores)
