...
class Chunk(object):
    def __init__(self, book_name, chunk_id, sentences, sbert_sentence_embeddings, doc2vec_chunk_embedding):
    ...
    def __find_bigram_counts(self):
        ...
        for i in range(len(splitted_processed_text) - 1):
            current_bigram = (splitted_processed_text[i], splitted_processed_text[i+1])
    ...
    def __find_trigram_counts(self):
        ...
        for i in range(len(splitted_processed_text) - 2):
            current_trigram = (splitted_processed_text[i], splitted_processed_text[i+1], splitted_processed_text[i+2])
