            ...
            for features in task_params['features']:
                ...
                for outer_fold in range(0, n_outer_folds):
                    fold_results = pd.read_csv(
                        os.path.join(gridsearch_dir, f'inner-cv-result_{language}_{task}_{label_type}_{features}_fold-{outer_fold}.csv'), 
                        header=0)
                    # Check if there are NaN harmonic p-values
                    if task == 'regression':
                        na_correlation_coefficient = fold_results[fold_results['harmonic_pvalue'].isnull()]
                        na_correlation_coefficient.to_csv(
                            os.path.join(results_dir, f'napvalue_{language}_{task}_{label_type}.csv'), 
                            index=False, 
                            header=True, 
                            na_rep='NaN')
