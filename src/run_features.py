# %%
'''
Classes for extracting text features and checking them.
'''
# %load_ext autoreload
# %autoreload 2
...
class FeatureProcessor(DataHandler):
    def __init__(self, language, by_author=False):
        super().__init__(language, 'features', test=False, by_author=by_author)
    ...
    def run(self):
        ...
        path_dcf = os.path.join(self.pickle_dir, 'doc_chunk.pkl')
        path_dbf = os.path.join(self.pickle_dir, 'doc_book.pkl')

        if os.path.exists(path_dcf) and os.path.exists(path_dbf):
            doc_chunk_features = self.load_pickle(path_dcf)
            doc_book_features = self.load_pickle(path_dbf)
        else:
            doc_chunk_features, doc_book_features = self.get_doc_features(as_chunk=True)
            self.save_pickle(path_dcf, doc_chunk_features)
            self.save_pickle(path_dbf, doc_book_features)
        ...
        start = time.time()
        path_ccf = os.path.join(self.pickle_dir, 'corpus_chunk.pkl')
        path_cbf = os.path.join(self.pickle_dir, 'corpus_book.pkl')
        start = time.time()
        if os.path.exists(path_ccf) and os.path.exists(path_cbf):
            corpus_chunk_features = self.load_pickle(path_ccf)
            corpus_book_features = self.load_pickle(path_cbf)
        else:
            corpus_chunk_features, corpus_book_features = self.get_corpus_features(as_chunk=True)
            self.save_pickle(path_ccf, corpus_chunk_features)
            self.save_pickle(path_cbf, corpus_book_features)
...
class FeatureChecker(DataHandler):
    def __init__(self, language, by_author=False):
        super().__init__(language, 'features', by_author=by_author)
